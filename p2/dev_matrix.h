...
public:
    ...
    explicit dev_matrix(size_t row, size_t col, size_t dep)
    {
        allocate(row*col*dep);
    }
    // destructor
    ~dev_matrix()
    {
    ...
    void resize(size_t row, size_t col, size_t dep)
    {
        free();
        allocate(row*col*dep);
    }
    ...
    void set(const T* src, size_t row, size_t col, size_t dep)
    {
        size_t min = std::min(row*col*dep, getSize());
        cudaError_t result = cudaMemcpy(start_, src, min * sizeof(T), cudaMemcpyHostToDevice);
        if (result != cudaSuccess)
        {
            throw std::runtime_error("failed to copy to device memory");
    ...
    void get(T* dest, size_t row, size_t col, size_t dep)
    {
        size_t min = std::min(row*col*dep, getSize());
        cudaError_t result = cudaMemcpy(dest, start_, min * sizeof(T), cudaMemcpyDeviceToHost);
        if (result != cudaSuccess)
